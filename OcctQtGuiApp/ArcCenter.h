...
public:
    ...
    //! Return arc center point. Debug output.
     gp_Pnt Arc_cp(bool debug){
         myDebug=debug;
         return Arc_cp();
     }
    ...
    gp_Pnt Arc_cp(){
        if(myDebug){std::cout<<"arc_cp x:"<<arc_cp.X()<<" y:"<<arc_cp.Y()<<" z:"<<arc_cp.Z()<<std::endl;}
        return arc_cp;
    }

    //! Return arc radius. Debug output.
    double Arc_radius(bool debug){
        myDebug=debug;
        return Arc_radius();
    }
    ...
    //! Return arc radius.
    double Arc_radius(){
        if(myDebug){std::cout<<"radius: "<<myRadius<<std::endl;}
        return myRadius;
    }

    //! Return arc angle in radians.
    double Arc_angle(bool debug){
        myDebug=debug;
        return Arc_angle();
    }
    ...
    //! Return arc angle in radians.
    double Arc_angle(){
        if(myDebug){std::cout<<"arc angle: "<<arc_angle<<std::endl;}
        return arc_angle;
    }
private:
    gp_Pnt p1, p2, p3, arc_cp;
    double myRadius, arc_angle;
    bool myDebug=0;
};
